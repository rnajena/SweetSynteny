...
"""
Modified postprocess_hmmscan.py
Links Pfam domains from representative protein sequences back to all MMseqs cluster members.
"""
import csv
import argparse
import os
...
def setup_parser():
    ...
    parser.add_argument('-hf', '--hmmscan_file', required=True, help='hmmscan tblout file')
...
def read_file(hmmscan_file):
    """Parses the hmmscan --tblout output."""
    results = []
    if not os.path.exists(hmmscan_file):
        return results
    ...
    with open(hmmscan_file, 'r') as f:
        for line in f:
            ...
            # Index 0: target_name (Pfam ID), Index 2: query_name (Representative ID)
            result = {
                'target_name': row[0],
...
def main():
    ...
    ensure_input_file(args.hmmscan_file)
    ...
    best_hits_dict = filter_hits(hmmscan_lst)
