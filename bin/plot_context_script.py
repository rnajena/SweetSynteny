...
def prepare_dataframe(input_file):
    ...
    df['orientation'] = df['orientation'].apply(map_ori)
    return df

def cluster_genomes(df):
    ...
    db = DBSCAN(eps=0.2, min_samples=5, metric='cosine').fit(feature_matrix_color)
    return db.labels_, feature_matrix_color

def plot_cluster(cluster_df, output_path, output_ending, scale, max_subplots=20):
    ...
    for subplot_idx in range(num_subplots):
        ...
        max_distance = compute_max_distance(subplot_genomes, scale)
        ...
        for i, (genome_name, genome_df) in enumerate(subplot_genomes):
            plot_genome(genome_df, axes[i], scale, max_distance)
...
def compute_max_distance(genome_groups, scale):
...
def plot_genome(genome_df, ax, scale, max_distance):
...
def main():
    ...
    try:
        df = prepare_dataframe(args.input_file)

        labels, feature_matrix = cluster_genomes(df)
        df["cluster_label"] = df["genome"].map(pd.Series(labels, index=feature_matrix.index))

        for cluster_label, cluster_df in df.groupby("cluster_label"):
            plot_cluster(cluster_df, args.output_path, args.output_ending, args.scale)
        #cluster stats as csv
        cluster_stats = []
        ...
        for cluster_label, cluster_df in df.groupby("cluster_label"):
            ...
            cluster_genomes = feature_matrix_color.loc[genome_to_cluster[genome_to_cluster == cluster_label].index]

            if cluster_genomes.shape[0] > 1:
                similarity_matrix = cosine_similarity(cluster_genomes.values)
        ...
        stats_df.to_csv(f"{output_path}_cluster_stats.csv", index=False)
...
--output_path /home/we93kif/maria_projects/SweetSynteny/bin/test \
--output_ending png
"""
